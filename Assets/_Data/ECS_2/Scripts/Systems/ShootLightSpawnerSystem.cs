...
partial struct ShootLightSpawnerSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        {
            if (!shootAttack.ValueRO.onShoot.isTriggered) continue;

            Entity shootLightEntity = state.EntityManager.Instantiate(entitiesReferences.shootLightPrefabEntity);
            SystemAPI.SetComponent(shootLightEntity, LocalTransform.FromPosition(shootAttack.ValueRO.onShoot.shootFromPos));
        }
    }
}
