...
partial struct ZombieSpawnerSystem : ISystem
{
    ...
    public void OnUpdate(ref SystemState state)
    {
        ...
        foreach ((RefRO<LocalTransform> localtrans, RefRW<ZombieSpawner> zombieSpawner) in
            SystemAPI.Query<RefRO<LocalTransform>, RefRW<ZombieSpawner>>())
        {
            ...
            SystemAPI.SetComponent(zombieEntity, LocalTransform.FromPosition(localtrans.ValueRO.Position));

            entityCommandBuffer.AddComponent(zombieEntity, new RandomWalking
            {
                originPos = localtrans.ValueRO.Position,
